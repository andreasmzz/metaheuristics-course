...
# inner_time_timit: may stop a SA before lowing temperature enough -> solution not good enough
def run_simulated_annealing_experiment(files:list[str], files_to_run:list[int], outer_time_limit:float, inner_time_limit:float, runs_per_file:int) -> None:
    ...
    for file_id in files_to_run:
        ...
        for run in range(runs_per_file):
            ...
            (useful_temp, starting_temp, beta, gamma) = sa.find_initial_temperature(
            ...
            (solution, benefit, initial_temp, alpha) = sa.simulated_annealing(
                sol = fs.create_randomic_solution(pack_benefits, dep_sizes, pack_dep, capacity),
                pack_benefits = pack_benefits,
                dep_sizes = dep_sizes,
                pack_dep = pack_dep,
                capacity = capacity,
                initial_temperature = useful_temp,
                time_limit = inner_time_limit - time.time() + inner_start_time
                )
            ...
            results.append({
                ...
                "benefit": benefit,
                "starting_find_temp": initial_temp, # tracking find temperature is that important?
