...
heuristic_type = Union[
    Callable[[list[bool], list[int], list[tuple[int, int]], list[str], float, int], move.move_type], # random, first and best
]

''' Functions '''

# Returns a randomic better solution with the move name and parameters that reached new_sol
def random_best_step(sol: list[bool], pack_benefits:list[int], pack_dep:list[tuple[int, int]], neighborhood_names:list[str] = [], time_limit: float = TIME_LIMIT_DEFAULT, max_tries: int = 1000) -> move.move_type:
...
def first_best_step(sol: list[bool], pack_benefits:list[int], pack_dep:list[tuple[int, int]], neighborhood_names:list[str] = [], time_limit: float = TIME_LIMIT_DEFAULT, max_tries: int = 1000) -> move.move_type:
    error_output: move.move_type = (sol, "error", -1)
    current_move_evaluation: int = evaluate_packs(pack_benefits, pack_dep, sol)
...
def absolute_best_step(sol: list[bool], pack_benefits:list[int], pack_dep:list[tuple[int, int]], neighborhood_names:list[str] = [], time_limit: float = TIME_LIMIT_DEFAULT, max_tries: int = 1000) -> move.move_type:
    error_output: move.move_type = (sol, "error", -1)
